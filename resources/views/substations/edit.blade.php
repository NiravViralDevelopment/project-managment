...
@section('title', 'Edit Sub Station')
...
@section('content')
    <nav aria-label="breadcrumb" class="mb-2">
        <ol class="breadcrumb mb-0">
            ...
            <li class="breadcrumb-item active">Edit Sub Station</li>
        </ol>
    </nav>
    <div class="mb-4">
        <h1 class="h2">Edit Sub Station</h1>
        <p class="text-muted mb-0">Update: {{ $substation->name }}.</p>
    </div>

    <div class="card shadow-sm">
        <div class="card-body">
            <form method="POST" action="{{ route('substations.update', $substation) }}" id="substationForm">
                @csrf
                @method('PUT')
                <div class="mb-3">
                    <label for="name" class="form-label">Name</label>
                    <input type="text" name="name" id="name" value="{{ old('name', $substation->name) }}" required class="form-control @error('name') is-invalid @enderror" placeholder="Sub station name">
                ...
                <div class="mb-3">
                    ...
                    <select name="zone_id" id="zone_id" required class="form-select @error('zone_id') is-invalid @enderror">
                        ...
                        @foreach($zones as $zone)
                            <option value="{{ $zone->id }}" {{ old('zone_id', $substation->zone_id) == $zone->id ? 'selected' : '' }}>{{ $zone->name }}</option>
                        @endforeach
                    </select>
                ...
                <div class="mb-3">
                    ...
                    <select name="circle_id" id="circle_id" required class="form-select @error('circle_id') is-invalid @enderror">
                        ...
                        @foreach($circles as $circle)
                            <option value="{{ $circle->id }}" {{ old('circle_id', $substation->circle_id) == $circle->id ? 'selected' : '' }}>{{ $circle->name }}</option>
                        @endforeach
                    </select>
                ...
                <div class="mb-4">
                    ...
                    <select name="division_id" id="division_id" required class="form-select @error('division_id') is-invalid @enderror">
                        ...
                        @foreach($divisions as $division)
                            <option value="{{ $division->id }}" {{ old('division_id', $substation->division_id) == $division->id ? 'selected' : '' }}>{{ $division->name }}</option>
                        @endforeach
                    </select>
                    @error('division_id')<div class="invalid-feedback">{{ $message }}</div>@enderror
                </div>
                <div class="d-flex gap-2">
                    <button type="submit" class="btn btn-primary">Update Sub Station</button>
    ...
    <script>
        $(function() {
            ...
            var currentCircleId = '{{ old("circle_id", $substation->circle_id) }}';
            var currentDivisionId = '{{ old("division_id", $substation->division_id) }}';

            function loadCircles(zoneId, selectCircleId, thenLoadDivisions) {
                if (!zoneId) {
                    circleSelect.html('<option value="">Select zone first</option>');
                    divisionSelect.html('<option value="">Select circle first</option>');
                    return;
                }
                ...
                $.get(circlesByZoneUrl, { zone_id: zoneId })
                    .done(function(circles) {
                        ...
                        if (thenLoadDivisions && circleSelect.val()) {
                            loadDivisions(circleSelect.val(), currentDivisionId);
                        } else {
            ...
            if (zoneSelect.val()) {
                loadCircles(zoneSelect.val(), currentCircleId, true);
            }
        });
    </script>
    @endpush
