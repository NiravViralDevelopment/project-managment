...
@section('content')
    ...
    <div class="mb-4">
        ...
        <p class="text-muted mb-0">Add a new project. Select Sub Station to filter users by hierarchy.</p>
    </div>
        ...
        <div class="card-body">
            <form method="POST" action="{{ route('projects.store') }}" id="projectForm">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name') }}" required class="form-control @error('name') is-invalid @enderror" placeholder="Project name">
                ...
                <div class="mb-3">
                    ...
                    <select name="substation_id" id="substation_id" class="form-select @error('substation_id') is-invalid @enderror">
                        ...
                        @foreach($substations as $s)
                            <option value="{{ $s->id }}" data-zone="{{ $s->zone?->name }}" data-circle="{{ $s->circle?->name }}" data-division="{{ $s->division?->name }}" {{ old('substation_id') == $s->id ? 'selected' : '' }}>{{ $s->name }} ({{ $s->zone?->name ?? '' }} / {{ $s->circle?->name ?? '' }} / {{ $s->division?->name ?? '' }})</option>
                        @endforeach
                    </select>
                ...
                <div class="row mb-3">
                    <div class="col-md-6">
                        ...
                        <input type="text" name="timeline" id="timeline" value="{{ old('timeline') }}" class="form-control @error('timeline') is-invalid @enderror" placeholder="e.g. Q1 2026 - Q4 2026">
                        @error('timeline')<div class="invalid-feedback">{{ $message }}</div>@enderror
                    </div>
                    <div class="col-md-6">
                        <label for="scheme" class="form-label">Scheme</label>
                        <input type="text" name="scheme" id="scheme" value="{{ old('scheme') }}" class="form-control @error('scheme') is-invalid @enderror" placeholder="Scheme name/number">
                ...
                <div class="row mb-3">
                    <div class="col-md-6">
                        ...
                        <input type="date" name="date_of_commissioning" id="date_of_commissioning" value="{{ old('date_of_commissioning') }}" class="form-control @error('date_of_commissioning') is-invalid @enderror">
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="date" name="scheduled_date_of_completion" id="scheduled_date_of_completion" value="{{ old('scheduled_date_of_completion') }}" class="form-control @error('scheduled_date_of_completion') is-invalid @enderror">
                ...
                <div class="mb-3">
                    ...
                    <input type="number" name="project_cost" id="project_cost" value="{{ old('project_cost') }}" step="0.01" min="0" class="form-control @error('project_cost') is-invalid @enderror" placeholder="0.00">
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <select name="status" id="status" required class="form-select @error('status') is-invalid @enderror">
                            @foreach(\App\Enums\ProjectStatus::cases() as $s)
                                <option value="{{ $s->value }}" {{ old('status', 'Active') === $s->value ? 'selected' : '' }}>{{ $s->value }}</option>
                            @endforeach
                        </select>
                ...
                <div class="mb-4">
                    ...
                    <select name="project_manager_id" id="project_manager_id" class="form-select @error('project_manager_id') is-invalid @enderror">
                        ...
                        @foreach($users as $u)
                            <option value="{{ $u->id }}" {{ old('project_manager_id') == $u->id ? 'selected' : '' }}>{{ $u->name }} ({{ $u->email }})</option>
                        @endforeach
                    </select>
