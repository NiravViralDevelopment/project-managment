...
@section('content')
    ...
    <div class="mb-4">
        ...
        <p class="text-muted mb-0">Update user and zone → circle → division → substation (user chain).</p>
    </div>
            ...
            <form method="POST" action="{{ route('users.update', $user) }}" id="userForm">
                ...
                <div class="mb-3">
                    ...
                    <select name="zone_id" id="zone_id" class="form-select @error('zone_id') is-invalid @enderror">
                        ...
                        @foreach($zones as $z)
                            <option value="{{ $z->id }}" {{ old('zone_id', $user->zone_id) == $z->id ? 'selected' : '' }}>{{ $z->name }}</option>
                        @endforeach
                    </select>
                ...
                <div class="mb-3">
                    ...
                    <select name="circle_id" id="circle_id" class="form-select @error('circle_id') is-invalid @enderror">
                        ...
                        @foreach($circles as $c)
                            <option value="{{ $c->id }}" {{ old('circle_id', $user->circle_id) == $c->id ? 'selected' : '' }}>{{ $c->name }}</option>
                        @endforeach
                    </select>
                ...
                <div class="mb-3">
                    ...
                    <select name="substation_id" id="substation_id" class="form-select @error('substation_id') is-invalid @enderror">
                        ...
                        @foreach($substations as $s)
                            <option value="{{ $s->id }}" {{ old('substation_id', $user->substation_id) == $s->id ? 'selected' : '' }}>{{ $s->name }}</option>
                        @endforeach
                    </select>
    ...
    <script>
        $(function() {
            ...
            var currentCircleId = '{{ old("circle_id", $user->circle_id) }}';
            var currentDivisionId = '{{ old("division_id", $user->division_id) }}';
            var currentSubstationId = '{{ old("substation_id", $user->substation_id) }}';
            ...
            function loadCircles(zoneId, selectCircleId, thenDivision, thenSubstation) {
                if (!zoneId) {
                    ...
                    substationSelect.html('<option value="">— Select division first —</option>');
                    return;
                }
                ...
                $.get(circlesByZoneUrl, { zone_id: zoneId }).done(function(circles) {
                    ...
                    if (thenDivision && circleSelect.val()) loadDivisions(circleSelect.val(), currentDivisionId, thenSubstation);
            ...
            function loadDivisions(circleId, selectDivisionId, thenSubstation) {
                ...
                $.get(divisionsByCircleUrl, { circle_id: circleId }).done(function(divisions) {
                    ...
                    if (thenSubstation && divisionSelect.val()) loadSubstations(divisionSelect.val(), currentSubstationId);
            ...
            if (zoneSelect.val()) loadCircles(zoneSelect.val(), currentCircleId, true, true);
        });
    </script>
    @endpush
@endsection
