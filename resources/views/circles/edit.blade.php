...
@section('content')
    <nav aria-label="breadcrumb" class="mb-2">
        <ol class="breadcrumb mb-0">
            ...
            <li class="breadcrumb-item active">Edit Circle</li>
        </ol>
    </nav>
    <div class="mb-4">
        <h1 class="h2">Edit Circle</h1>
        <p class="text-muted mb-0">Update circle: {{ $circle->name }}.</p>
    </div>
        ...
        <div class="card-body">
            <form method="POST" action="{{ route('circles.update', $circle) }}">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name', $circle->name) }}" required class="form-control @error('name') is-invalid @enderror" placeholder="Circle name">
                ...
                <div class="mb-4">
                    ...
                    <select name="zone_id" id="zone_id" required class="form-select @error('zone_id') is-invalid @enderror">
                        ...
                        @foreach($zones as $zone)
                            <option value="{{ $zone->id }}" {{ old('zone_id', $circle->zone_id) == $zone->id ? 'selected' : '' }}>{{ $zone->name }}</option>
                        @endforeach
                    </select>
